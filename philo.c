...
//	printf ("---->%ld---->%ld-->%d\n", die[i] - check_point[i] - time_to_eat , i, (*end));
		...
		if (/*die[is] - (time_to_eat + check_point)*/dead[is] <= time_to_eat)
		{
			pthread_mutex_lock(&mutexfork[is]);
	printf ("-\x1B[36m--->%ld--> die = %ld --check_point = %ld ---time_to_eat = %d---->%d\n\x1B[37m", dead[is] - time_to_eat /*- time_to_eat */,die[is], check_point , time_to_eat, is);
			(*breaker) = 0;
			(*re)[1] = 0;
...
while (str->breaker)
{
	...
	if (is % 2 == 0)
	{
		...
		if (/*die[is] <= 0*/time_to_die - check_point2[is] <= time_to_eat/*check_point2[is]*/ || str->breaker == 0)
		{
			oneortwo = 2;
			str->breaker = 0;
			pthread_mutex_unlock(&mutexfork[right]);
	...
	else
	{
		...
		current_time = ((ct.tv_sec * 1e6) + ct.tv_usec) / 1000 - start_time;
		check_point[is] = current_time - check_point[is];
		if (/*die[is] <= 0*//*check_point[is]*/ time_to_die - check_point[is] <= time_to_eat|| str->breaker == 0)
		{
			oneortwo = 1;
			str->breaker = 0;
			pthread_mutex_unlock(&mutexfork[right]);
		...
		current_time = ((ct.tv_sec * 1e6) + ct.tv_usec) / 1000 - start_time;
		check_point2[is] = current_time - check_point2[is];
		if (/*die[is] <= check_point2[is]*/ time_to_die - check_point2[is] <= time_to_eat|| str->breaker == 0)
		{
			oneortwo = 2;
			str->breaker = 0;
			pthread_mutex_unlock(&mutexfork[right]);
...
//		printf ("checkpoint2[%d] = %ld\n", is, check_point2[is]);
	if (/*die[is] <= 0*//*check_point2[is]*/time_to_die - check_point2[is] <= time_to_eat || str->breaker == 0)
	{
			oneortwo = 2;
		str->breaker = 0;
		break;
